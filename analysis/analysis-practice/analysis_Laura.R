...
# inference tests whether error decreases over iterations

# pitch interval error

pitch_error_model <- lm(mean_pitch_interval_error ~ degree, error_data)
summary(pitch_error_model)

# model is significant: F(1,9) = 24.5, p < .001, R-squared = .7313

ISI_error_model <- lm(mean_ISI_error ~ degree, error_data)
summary(ISI_error_model)

# model is not significant: F(1,9) = 4.159, p = .072, R-squared = .3161

# inclusion of tonality for potential interaction effects

# As I couldn't use the error data, I just used the whole dataset, so not the mean values.
# Should I also do this for the simple error analyses above?

pitch_error_model_t <- lm(root_mean_squared_interval ~ degree * mode, data_melodies)
summary(pitch_error_model_t)

# model is  significant: F(3,2191) = 16, p < .001, but: only degree is a significant predictor (mode and interaction ns)

ISI_error_model_t <- lm(ISI_rms_error ~ degree * mode, data_melodies)
summary(ISI_error_model_t)

# model is  significant: F(3,2191) = 6.93, p < .001, but: only degree is a significant predictor (mode and interaction ns)
...
# impact of tonality (major/minor mode)
...
plot_kde_by_mode
ggsave("distribution_intervals_by_mode.png", width = 14, height = 7, units = "cm")
...
# D = 0.036, p = .433 -- no significant difference between major and minor interval distributions

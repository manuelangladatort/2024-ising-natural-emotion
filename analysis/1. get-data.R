...
################################################################################
# TONALITY + EMOTION STUDY (imitation)
# iterated singing (naturalistic) - 200 nodes (across-chains)
# This study is shared between the tonality project and the emotion project
################################################################################
...
################################################################################
# TONALITY SUTDY (Varying number of notes)
# iterated singing (naturalistic) - 200 nodes (across-chains), 
# starting melodies from 4 to 13 notes (20 melodies in each condition)
################################################################################
...
data_nets = prepare_trial_data_freenotes(data_nodes[, -1], data_trials[, -1])

length(unique(data_nets$participant_id)) # 124
length(unique(data_nets$network_id)) # 200
length(unique(data_nets$participant_id)) 
length(unique(data_nets$network_id)) # 200
table(data_nets$degree) 
table(data_nets$trial_maker_id) # 2400 
...
################################################################################
# EMOTION STUDY (imitation with emotion): iterated singing (naturalistic + emotion) - 200 nodes (across-chains)
################################################################################
data_nodes <- read_csv("data/final-data/sing-emotion-v1/sing-emotion-v1_node_data.csv")
data_trials <- read_csv("data/final-data/sing-emotion-v1/sing-emotion-v1_trial_data.csv")
...
length(unique(data_nets$participant_id)) # 141
length(unique(data_nets$network_id)) # 200 
table(data_nets$degree) # not all iterations are completed with 200 networks...
table(data_nets$trial_maker_id) # 2182
...
data_nets %>% 
  rowwise() %>% 
  mutate_if(is.list, ~paste(unlist(.), collapse = ',')) %>% 
  write.csv("data/final-data/sing-emotion-v1/data-sing-emotion-v1_full.csv", row.names = FALSE)

################################################################################
# Old code to round and transpose the data
